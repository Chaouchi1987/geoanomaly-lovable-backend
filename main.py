...
app = FastAPI(title="GeoAnomaly Lovable Backend", version="0.2.0")
...
def init_ee():
    ...
    with EE_LOCK:
        try:
            ...
            ee.Number(1).getInfo()
            EE_READY = True
            EE_INFO = {"status":"ready","connected":True,"mode":mode,"project":project,"message":"Earth Engine initialized successfully"}
...
def bbox(lat, lon, radius_m):
    ...
    dlon = radius_m / max(1e-9, 111320.0 * abs(__import__("math").cos(__import__("math").radians(lat))))
...
@app.get("/")
def root():
    return {"service":"GeoAnomaly Lovable Backend","version":"0.2.0","policy":"No synthetic scientific results"}

@app.get("/health")
def health():
    return {"status":"ok","service":"geoanomaly-backend","version":"0.2.0"}
...
def create_aoi(p: AOIRequest):
    ...
    item = {
        ...
        "radius_m":p.radius_m, "scale_m":p.scale_m,
        "geometry_type":gtype, "name":p.name,
...
def run_analysis(run_id: str, request: dict):
    run = RUNS[run_id]
    try:
        state = init_ee()
        ...
        stats = image.select(["B2","B3","B4","B8","B11"]).reduceRegion(
            reducer=ee.Reducer.mean(), geometry=geom, scale=int(request.get("scale_m") or 10),
