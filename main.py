...
def _install_oauth_credentials(credentials_json: str) -> str:
    """
    Install Earth Engine OAuth credentials in the standard persistent
    credentials location used by earthengine-api.

    The JSON comes from the local file created by:
        earthengine authenticate

    It is never returned by the API and is never logged.
    """
    ...
    if missing:
        raise ValueError(
            "EARTHENGINE_CREDENTIALS_JSON is missing required fields: "
            + ", ".join(sorted(missing))
        )
...
def init_ee() -> dict[str, Any]:
    ...
    Priority:
      1. EARTHENGINE_CREDENTIALS_JSON
      2. GOOGLE_SERVICE_ACCOUNT_JSON
      3. Application Default Credentials

    OAuth credentials are installed in the standard Earth Engine persistent
    credentials path before calling ee.Initialize(credentials='persistent').
    """
    ...
    with EE_LOCK:
        try:
            ...
            if oauth_json:
                _install_oauth_credentials(oauth_json)

                # Earth Engine's documented persistent OAuth path reads the
                # credential file from ~/.config/earthengine/credentials.
                credentials = ee.data.get_persistent_credentials()

                if credentials is None:
                    raise RuntimeError(
                        "Earth Engine persistent OAuth credentials could not "
                        "be loaded."
                    )
            ...
            # Real server-side Earth Engine check.
            result = ee.Number(1).add(2).getInfo()

            if result != 3:
                raise RuntimeError(
                    "Earth Engine connectivity test returned an unexpected result."
                )
...
def bbox(lat: float, lon: float, radius_m: float) -> list[float]:
    import math
    dlat = radius_m / 111320.0
    dlon = radius_m / max(
        1e-9,
        111320.0 * abs(math.cos(math.radians(lat))),
    )
    return [lon - dlon, lat - dlat, lon + dlon, lat + dlat]


def geometry_for(aoi: dict[str, Any]):
    import ee
...
def create_aoi(p: AOIRequest):
    ...
    item = {
        ...
        "name": p.name,
        "area_m2": (
            3.141592653589793 * p.radius_m * p.radius_m
            if gtype == "circle"
            else (2 * p.radius_m) ** 2
        ),
        "bbox": bbox(p.latitude, p.longitude, p.radius_m),
        "created_at": now(),
    }
    AOIS[aoi_id] = item
    return item
...
def run_analysis(run_id: str, request: dict[str, Any]):
    run = RUNS[run_id]
    try:
        state = init_ee()
        ...
        run.update(
            ...
            stage="acquisition",
            progress=0.1,
            message="Querying Sentinel-2 collection",
        )
        ...
        run.update(
            stage="spectral_dem",
            progress=0.35,
            message="Computing real AOI summary statistics",
        )
...
def analysis_status(analysis_id: str):
    ...
    return {
        k: r.get(k)
        for k in [
            "analysis_id", "status", "stage", "progress",
...
def samples(analysis_id: str):
    return {
        "samples": get_run(analysis_id)["samples"],
...
def sentinel2_test(body: dict):
    ...
    if not state.get("connected"):
        raise HTTPException(
            503,
            state.get("message", "Earth Engine unavailable"),
        )
